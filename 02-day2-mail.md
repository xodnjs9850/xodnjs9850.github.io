...
- **POP3**: 받으면 서버에서 사라짐 (한 번만 다운로드)
- **IMAP**: 서버에 그대로. 모바일·PC·스크립트가 동시에 같은 받은 편지함을 봐도 일관됨

OpenClaw 에이전트도 같은 IMAP을 쓰므로, 메일을 분류·이동해도 폰 메일 앱과 결과가 동기화됩니다.
